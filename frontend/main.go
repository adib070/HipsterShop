...
func mustMapEnv(target *string, envKey string) {
	if envKey == "PRODUCT_CATALOG_SERVICE_ADDR" {
	...
	if envKey == "CURRENCY_SERVICE_ADDR" {
		*target = "currency:9000"
	}

	if envKey == "SHIPPING_SERVICE_ADDR" {
		*target = "shipping:50051"
	}

	if envKey == "AD_SERVICE_ADDR" {
		*target = "ad:9555"
	}

	if envKey == "RECOMMENDATION_SERVICE_ADDR" {
		*target = "recommended:8080"
	}
